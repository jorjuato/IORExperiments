...
char data_base[128]="../datos/rawdata/ior_discreto";		// Nombre del archivo donde se guardarán los resultados
		...
		{	
			...
			{	
				...
				{	
					...
					} 		//END_IF (trial < TRIAL_TRAIN_NUMBER)
				}			//END_FORs
			}				//
		}					//	
	}						//END_FORs
	...
	switch(g_trial_state)
	{
		...
		case TRIAL_STATE_SHOW_TARGET:///////////////////////////////////////////			
			if(g_bKeyStroke) //Primero comporbamos que no se haya pulsado tecla
			{
				...
				if((Resultados[g_trial_number].order % 50) == 0 )
					g_trial_state = TRIAL_STATE_TAKE_A_REST;
				else
					g_trial_state = TRIAL_STATE_INTER_TRIAL; 								
			}
...
static void ExperimentSave() {
	...
	else
	{
		...
		for(i=0;i<DIMENSION;i++)
			fprintf(data, "\t%d\t\t\t%d\t\t\t%s\t\t\t%d\t\t\t%d\t\t\t%d\t\t\t\t%d  \n",i 
														,(int)Resultados[i].order
														,Resultados[i].type 
														,(int)Resultados[i].ctoa  
														,Resultados[i].ctd.value
														,(int)Resultados[i].rt
