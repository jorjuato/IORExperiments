...
#define MAXPUNTOS			3
#define TICK_INTERVAL		5		// Tiempo de muestreo de la tableta en milisegundos
...
void WacomTimer(int value) {
	...
	fprintf(archivo,"%d \t %d  \t %d \t %d \t %d \t %f \n", g_bufferx[contador], g_buffery[contador],gAbsState[WACOMFIELD_POSITION_X].nValue, gAbsState[WACOMFIELD_POSITION_Y].nValue, gAbsState[WACOMFIELD_PRESSURE].nValue, (double)glutGet(GLUT_ELAPSED_TIME));
