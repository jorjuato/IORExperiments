...
char *imgfile="ball.bmp";				// Nombre de archivo de la imagen del target
char *datafile="data.txt";				// Nombre del archivo donde se guardarán los resultados


//////////////////////////////////////////////////// IOR GLOBAL STATE VARIABLES
Uint8 g_trial_type;								// Actual type of trial, value from enum TRIAL_TYPE
			...
			{	
				...
				{	
					if (trial < TRIAL_TRAIN_NUMBER)
					{
						...
						Resultados[trial].ctd  = ctd_val[j] ;
						Resultados[trial].type = k ;
						while(TRUE)
						{
...
static void ExperimentSave() {
	...
	{
		fprintf(data, "#$ \t TRIAL_LEFT_LEFT \t=\t %d\n", TRIAL_LEFT_LEFT);
		fprintf(data, "#$ \t TRIAL_LEFT_RIGHT \t=\t %d\n", TRIAL_LEFT_RIGHT);
		fprintf(data, "#$ \t TRIAL_RIGHT_LEFT \t=\t %d\n",TRIAL_RIGHT_LEFT);
		fprintf(data, "#$ \t TRIAL_RIGHT_RIGHT \t=\t %d\n\n",TRIAL_RIGHT_RIGHT);
		
		fprintf(data, "#$ \t TRIAL_TRAIN_NUMBER =\t %d\n",TRIAL_TRAIN_NUMBER);
		fprintf(data, "#$ \t TRIAL_TYPE_MAX \t=\t %d\n",TRIAL_TYPE_MAX);
		fprintf(data, "#$ \t TRIAL_REPLICATION  =\t %d\n\n",TRIAL_REPLICATION);
