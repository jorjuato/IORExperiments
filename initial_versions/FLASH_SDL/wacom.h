...
//
//				gcc archivos.c -L. -lwacom -lpthread a.out
//
//  
...
//#ifndef _STATE_STRUCTURES_
//#define _STATE_STRUCTURES_
struct ABS_STATE
{
	int bValid;
...
void WacomInit(int Modo, void (*WacomSignalHandler)(int));  // Inicializa y configura la tableta, 
															 // lanza el thread wacom y configura el
															 // handler de señal SIGIO 
void WacomClose(void); // Libera recursos de la tableta y gráficos, termina el thread, 
					   // y cierra los archivos abiertos para escritura. 
					   
/* Ejemplo de Manejador de señales para los eventos wacom
void WacomSignalHandler() {
	...
	signal(SIGIO,WacomSignalHandler);			
