 ...
 void ShowTarget()
{
	...
	{		
		dst.x = (int) (CUE_LEFT_X - src.w / 2) + Resultados[g_trial_number].ctd.value;
		dst.y = (int) (CUE_Y - src.h / 2);  
		dst.w = src.w; 
		dst.h = src.h;	
	}
	else if (g_trial_type == "LR" || g_trial_type == "RR")
	{		
		dst.x = (int) (CUE_RIGHT_X - src.w / 2) + Resultados[g_trial_number].ctd.value;
		dst.y = (int) (CUE_Y - src.h / 2);  
		dst.w = src.w; 
		dst.h = src.h;	
	}
